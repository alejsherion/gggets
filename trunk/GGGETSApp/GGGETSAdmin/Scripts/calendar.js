<!--
var rttime=false;
//document.write("<div id=meizzCalendarLayer style='position: absolute;z-index: 9999; width: 1; height: 193;display: none'>");
//document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=145px height=193px></iframe></div>");
...
function writeIframe()
{
    ...
    "    </td><td align=center  colspan=2  ><input name=dayclear type=button onclick=\"parent.returnDateClear()\" value='清空' /><input name=today type=button value='今天' onfocus='this.blur()'  title='当前日期&#13;快捷键：T'"+
...
function calendars(isTrueFlase) //主调函数2
{
    if(isTrueFlase=="true")
    {
        rttime=true;
...
function returnDateClear() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
