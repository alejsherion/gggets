...
{
    public partial class AddRole_Privilege : System.Web.UI.Page
    {
         #region 构造函数以及字段
        ...
        protected AddRole_Privilege()
        {
            
        }
        public AddRole_Privilege(IAppModuleManagementService appModuleManagementService
        ...
        protected void trvwAppModule_NodeDataBound(object sender, Telerik.Web.UI.RadTreeNodeEventArgs e)
        {
            ...
            var dataSource = (IList<AppModule>) ViewState["AppModel"];
        ...
        private void GetDataSourceFormTree(Role role)
        {
            ...
            role.Role_Privilege.Clear();
            var nodes = trvwAppModule.CheckedNodes;
            if(nodes!=null&&nodes.Count>0)
            {
                ...
                foreach(RadTreeNode node in nodes)
                {
                    if(node.Nodes.Count==0)
                    {
                        ...
                        if(ht.ContainsKey(modelId))
                        {
                            rolePrivilege = (Role_Privilege) ht[modelId];
                            if (rolePrivilege == null) continue;
